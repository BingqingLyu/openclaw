...
// file-type returns undefined (no magic bytes) for text-based formats like CSV,
// Markdown, HTML, XML, and CSS, so host-read needs an explicit "this really
// decodes as text" fallback. Binary or ZIP payloads renamed with these
// extensions are rejected by isValidatedHostReadText (printable-ratio +
// no-binary-sniff check).
...
function assertHostReadMediaAllowed(params: {
  ...
  // For extension-declared plain-text aliases such as .csv/.md/.html, trust only the
  ...
  if (declaredMime && HOST_READ_TEXT_PLAIN_ALIASES.has(declaredMime)) {
    const sniffed = normalizeMimeType(params.sniffedContentType);
    const sniffOk = !sniffed || HOST_READ_TEXT_PLAIN_ALIASES.has(sniffed);
    if (sniffOk && params.buffer && isValidatedHostReadText(params.buffer)) {
      return;
    }
    throw new LocalMediaAccessError(
      "path-not-allowed",
      "hostReadCapability permits only validated text documents (CSV/Markdown/HTML/XML/CSS) for local reads",
    );
  }
  ...
  // Text-document exception: file-type v22 returns undefined for these
  // plain-text buffers because they have no binary magic bytes. Allow them when:
  // - sniffedMime is undefined (no binary signature detected by file-type)
  // - The extension-derived MIME is one of the explicit text-document aliases
